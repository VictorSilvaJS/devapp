# Fechamento Da Frente Visitas E Caderno Por Fazenda

Status em 2026-05-19: a frente funcional de visitas tecnicas por fazenda e caderno de campo por fazenda foi validada no nivel necessario para o MVP atual.
...
- ao abrir `NovoCaderno` por rota direta com uma fazenda sem permissao, o formulario aparecia rapidamente antes da tela de bloqueio.

Ajuste aplicado:

- enquanto as fazendas autorizadas carregam, a tela mostra estado de carregamento;
- se a fazenda da rota nao for permitida, a tela vai para `Acesso restrito` sem exibir o formulario antes.
...
### Recarregamento Do Detalhe Da Fazenda

Arquivo ajustado: `src/screens/ProdutorScreen.tsx`.

Problema encontrado:

- ao criar um caderno pela aba Caderno da fazenda, o registro era salvo e o detalhe abria, mas ao voltar para a fazenda a aba podia continuar com a lista antiga.

Ajuste aplicado:

- o detalhe da fazenda recarrega seus dados sempre que a tela recebe foco e existe `id` na rota.

Impacto:

- registros criados pela aba Caderno passam a aparecer corretamente quando o usuario volta para o detalhe da fazenda.
...
- Produtor nao acessa caderno de outra fazenda.
- Produtor nao cria caderno em fazenda de outro titular.
...
- Edicao de visita preserva a fazenda vinculada.
- Edicao de caderno preserva a fazenda vinculada.
- Campos de fazenda em edicao aparecem travados.
- Criacao de visita salva no contexto da fazenda selecionada.
- Criacao de caderno salva no contexto da fazenda selecionada.
- Criacao de caderno pela aba da fazenda nasce no contexto da fazenda atual.

### Integracao No Detalhe Da Fazenda

- Aba Caderno mostra registros da fazenda atual.
- Aba Caderno respeita visibilidade para produtor.
- Novo registro criado pela aba aparece na aba da fazenda apos retorno.
...
- criar uma entrada de visitas dentro de `Minhas Fazendas`;
- criar uma secao de visitas dentro do detalhe da fazenda;
...
A frente de visitas tecnicas por fazenda e caderno de campo por fazenda pode ser considerada encerrada no nivel MVP.

O fluxo esta validado para admin, colaborador e produtor, incluindo bloqueios por rota direta, escopo regional, preservacao de `fazenda_id`, visibilidade de caderno para produtor e criacao contextual a partir do detalhe da fazenda.
