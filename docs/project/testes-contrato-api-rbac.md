...
Status revisado em 2026-08-05: este documento define uma matriz tecnica de
testes de contrato/API baseada em `contrato-api-rbac.md` e nas decisoes 31 a
33. Ele nao implementa backend. Enquanto a migracao nao terminar, o runtime v1
ainda pode conter campos regionais que nao pertencem ao contrato alvo.
...
| API-RBAC-PROP-07 | Colaborador sem vinculo tenta abrir Propriedade | Colaborador | Sem `usuario_propriedade` ativo | `GET /propriedades/:id` | Nao se aplica | `403 Forbidden` ou `404 Not Found` | Vinculo direto ativo e obrigatorio | Automatizado backend/API |
...
| API-RBAC-PROP-09 | Admin cria Propriedade | Admin | Payload valido | `POST /propriedades` | `{ "nome": "...", "titular_produtor_id": "...", "municipio": "...", "uf": "RS", "area_total": 120.5, "status": "ativa" }` | `201 Created` | Admin cria cadastro | Automatizado backend/API |
...
| API-RBAC-MAPA-05 | Colaborador tenta anexos sem vinculo direto | Colaborador | Sem vinculo direto ativo | `GET /propriedades/:id/anexos` | Filtros opcionais | `403 Forbidden` ou `404 Not Found` | Localizacao coincidente nao concede acesso | Automatizado backend/API |
...
| API-RBAC-VIS-07 | Colaborador cria visita fora do escopo | Colaborador | Sem vinculo direto ativo | `POST /visitas` | Payload minimo valido | `403 Forbidden` ou `404 Not Found` | Rotas diretas nao burlam escopo | Automatizado backend/API |
...
| API-RBAC-CAD-07 | Colaborador cria caderno fora do escopo | Colaborador | Sem vinculo direto ativo | `POST /caderno` | Payload minimo valido | `403 Forbidden` ou `404 Not Found` | Escopo validado no backend | Automatizado backend/API |
